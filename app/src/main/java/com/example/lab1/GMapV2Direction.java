...
public class GMapV2Direction extends AsyncTask<String, Void, Document> {
    ...
    public static final java.lang.String MODE_CYCLING = "bicycling";
    private String API_key;
    private Context context;

    public GMapV2Direction(String key, Context context) {
        this.API_key = key;
        this.context = context;
    }

    //direction = start.latitude, start.longitude, end.latitude, end.longitude, mode
    @Override
    protected Document doInBackground(String[] direction){

        String start_latitude = direction[0];
        String start_longitude = direction[1];
        String end_latitude = direction[2];
        String end_longitude = direction[3];
        String mode = direction[4];
    ...
    public String getDurationText(Document doc) {
        try {
            ...
            Node node2 = nl2.item(getNodeIndex(nl2, (String) "text"));
            Log.i("DurationText", node2.getTextContent());
            return (String) node2.getTextContent();
        } catch (Exception e) {
            return (String) "0";
        }
    }
    ...
    public int getDurationValue(Document doc) {
        try {
            ...
            Node node1 = nl1.item(0);
            NodeList nl2 = node1.getChildNodes();
            Node node2 = nl2.item(getNodeIndex(nl2, (String) "value"));
    ...
    public String getStartAddress(Document doc) {
        try {
            ...
            return (String) node1.getTextContent();
        } catch (Exception e) {
            return (String) "-1";
        }
        ...
        if (nl1.getLength() > 0) {
            for (int i = 0; i < nl1.getLength(); i++) {
                ...
                Node locationNode = nl2
                        .item(getNodeIndex(nl2, (String) "start_location"));
                nl3 = locationNode.getChildNodes();
                Node latNode = nl3.item(getNodeIndex(nl3, (String) "lat"));
                double lat = Double.parseDouble(latNode.getTextContent());
                Node lngNode = nl3.item(getNodeIndex(nl3, (String) "lng"));
                double lng = Double.parseDouble(lngNode.getTextContent());
                listGeopoints.add(new LatLng(lat, lng));

                locationNode = nl2.item(getNodeIndex(nl2, (String) "polyline"));
                nl3 = locationNode.getChildNodes();
                latNode = nl3.item(getNodeIndex(nl3, (String) "points"));
                ArrayList<LatLng> arr = decodePoly((java.lang.String) latNode.getTextContent());
                for (int j = 0; j < arr.size(); j++) {
                    listGeopoints.add(new LatLng(arr.get(j).latitude, arr
                            .get(j).longitude));
                }

                locationNode = nl2.item(getNodeIndex(nl2, (String) "end_location"));
                nl3 = locationNode.getChildNodes();
                latNode = nl3.item(getNodeIndex(nl3, (String) "lat"));
                lat = Double.parseDouble(latNode.getTextContent());
                lngNode = nl3.item(getNodeIndex(nl3, (String) "lng"));
    ...
    private ArrayList<LatLng> decodePoly(java.lang.String encoded) {
